...
public class BedChair extends RotatingChair {
    ...
    public BedChair(Player player, Block block) {
        super(ChairType.BED, player, block, block.getLocation().clone().add(Chair.OFFSETS.apply(-0.45D)));
        if (!(block.getBlockData() instanceof Bed)) throw new IllegalArgumentException("This is not bed: " + block);
        ...
        if (worldTicks >= MIN_TIME_TICK && worldTicks <= MAX_TIME_TICK)
            throw new IllegalArgumentException("This world is in sleeping time: " + block);
    }
}
