...
public class StairChair extends Chair {
	...
	public StairChair(Player player, Block block) throws Exception {
		...
		if(block.getBlockData() instanceof Stairs == false) throw new Exception("This is not stairs: " + block.toString());
		this.stairs = (Stairs) block.getBlockData();
		if(stairs.getHalf() != Half.BOTTOM) throw new Exception("Stairs half is not bottom: " + block.toString());
		if(stairs.getShape() != Shape.STRAIGHT) throw new Exception("Stairs shape is not straight: " + block.toString());
		...
		else throw new Exception("Stairs facing not include: " + block.toString());
